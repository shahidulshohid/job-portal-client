    ...
    <div className="card card-compact bg-base-100 shadow-xl">
      ...
      <div className="card-body">
        ...
        <div className="flex flex-wrap gap-2">
            {
                requirements.map((skill, idx) => <p key={idx} className="border rounded-md p-2 text-center hover:text-purple-600 hover:bg-gray-400">{skill}</p>)
            }
        </div>
        <div className="card-actions justify-end items-center mt-4">
          ...
          <Link to={`/job/${_id}`}><button className="btn btn-primary">Apply</button></Link>
        </div>
      </div>
    </div>
  );
